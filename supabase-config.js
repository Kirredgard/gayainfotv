...
(function () {
  ...
  /* ============================================================
     ...
     Définis AVANT initSupabase pour être disponibles dès le premier emit()
  ============================================================ */
  window.gayaCommentCounts = {};
  window.gayaViewCounts = {};

  window.gayaGetCommentCount = function(id) {
    return Number(window.gayaCommentCounts[String(id)] || 0);
  };
  window.gayaGetViewCount = function(id) {
    return Number(window.gayaViewCounts[String(id)] || 0);
  };
  ...
  window.gayaRefreshCommentCounts = async function(articleIds) {
    ...
    if (!_client) { ids.forEach(id => _pendingCommentCountIds.add(id)); return; }
    const { data, error } = await _client.from(COMMENTS_TABLE).select("article_id,parent_id").in("article_id", ids);
  ...
  window.gayaRefreshViewCounts = async function(articleIds) {
    const ids = normalizeIds(articleIds);
    if (!ids.length) return;
    if (!_client) { ids.forEach(id => _pendingViewCountIds.add(id)); return; }
    ...
    document.querySelectorAll("[data-view-count-id]").forEach(el => {
      el.textContent = String(window.gayaGetViewCount(el.getAttribute("data-view-count-id")));
    });
  };
  ...
  async function initSupabase() {
    try {
      if (!GAYA_SUPABASE_URL || GAYA_SUPABASE_URL.includes("TON-PROJET") ||
          !GAYA_SUPABASE_ANON_KEY || GAYA_SUPABASE_ANON_KEY.includes("REMPLACE")) {
      ...
      if (_pendingCommentCountIds.size) {
        const ids = Array.from(_pendingCommentCountIds); _pendingCommentCountIds.clear();
        window.gayaRefreshCommentCounts(ids);
      }
      if (_pendingViewCountIds.size) {
        const ids = Array.from(_pendingViewCountIds); _pendingViewCountIds.clear();
  ...
  window.gayaCMSOnUpdate = function(cb) { if (typeof cb === "function") _listeners.push(cb); if (_remoteData) cb(_remoteData); };
  ...
  window.gayaCMSLogin = async function(email, password) {
    ...
    return { ok: true, user, displayName: user.user_metadata?.full_name || user.email || "Administrateur", role: "Administrateur" };
  };
  ...
  window.gayaCMSWriteComments = async function() {
    console.warn("[GAYA CMS] gayaCMSWriteComments obsolète — utiliser article.js/postComment.");
  };
